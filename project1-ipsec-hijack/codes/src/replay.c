...
void tx_esp_rep(Dev dev,
                ...
                Esp esp,
                Txp txp,
                uint8_t *data, ssize_t dlen, long msec,bool mode)
{
    size_t nb = dlen;

    txp.plen = dlen;
    txp.fmt_rep(&txp, net.ip4hdr, data, nb,mode);
                   ...
                   char* str,bool mode)
{
    ...
    tx_esp_rep(*dev, *net, *esp, *txp, buf, nb, 0,mode);
    return nb;
}
                ...
                bool mode)
{
    ...
    if (net->pro == ESP) {
        ...
        uint8_t *txp_data = txp->dissect(net, txp, esp_data, esp->plen,mode);
                  ...
                  bool mode)
{
    ...
    while (true) {
        ...
        dissect_finish = dissect_rx_data(dev, net, esp, txp, state, victim_ip, server_ip, test_for_dissect,mode) ? true : false;
...
void get_info(Dev *dev, Net *net, Esp *esp, Txp *txp,int *state,char* victim_ip,char* server_ip,bool* test_for_dissect,bool mode)
{
    extern EspHeader esp_hdr_rec;

    wait(dev, net, esp, txp, state, victim_ip, server_ip, test_for_dissect,mode);
